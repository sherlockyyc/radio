...
class PIM_FGSM(BaseMethod):
    ...
    Args:
        ...
        self.criterion ([]): 损失函数
    """
    def __init__(self, model, criterion = None, use_gpu = False, device_id = [0], eps=0.03, shift = 20, sample_num = 10, is_target=False, target=0):
        """[summary]
        ...
        """
        super(PIM_FGSM,self).__init__(model = model, criterion= criterion, use_gpu= use_gpu, device_id= device_id)
        self.eps = eps
    ...
    def attack(self, x, y=0):
        ...
        self.model.eval()
        if self.is_target:
            x_adv,pertubation = self._attackWithTarget(x, self.target, self.eps, self.shift, self.sample_num )
            message = "At present, we haven't implemented the Target attack algorithm "
            assert x_adv is not None,message
        else:
            x_adv,pertubation = self._attackWithNoTarget(x, y,self.eps, self.shift, self.sample_num )
    ...
    def _attackWithNoTarget(self, x, y, eps, shift, sample_num ):
        x_adv = x
    ...
    def _attackWithTarget(self, x, target, eps, shift, sample_num ):

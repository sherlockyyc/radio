...
class PIM_FGSM(BaseMethod):
    ...
    def attack(self, x, y=0, x_snr=[], eps=0.03, is_target=False, target=0, mu = 1, shift = 20, sample_num = 10):
        """[summary]
        ...
        self.model.eval()
        if is_target:
            x_adv,pertubation = self._attackWithTarget(x, target, eps, mu, shift, sample_num )
            message = "At present, we haven't implemented the Target attack algorithm "
            assert x_adv is not None,message
        else:
            x_adv,pertubation = self._attackWithNoTarget(x, y, eps, mu, shift, sample_num )
    ...
    def _attackWithNoTarget(self, x, y, eps, mu, shift, sample_num ):
        x_adv = x
    ...
    def _attackWithTarget(self, x, target, eps, mu, shift, sample_num ):
